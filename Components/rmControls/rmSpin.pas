...
function TrmCustomSpinEdit.IsValidChar(var Key: Char): Boolean;
begin
   result := false;
end;
...
function TrmSpinEdit.IsValidChar(var Key: Char): Boolean;
begin
...
function TrmFloatSpinEdit.IsValidChar(var Key: Char): Boolean;
begin
  Result := (Key in [DecimalSeparator, '+', '-', '0'..'9']) or
    ((Key < #32) and (Key <> Chr(VK_RETURN)));
  if not FEditorEnabled and Result and ((Key >= #32) or
...
function TrmTimeSpinEdit.IsValidChar(var Key: Char): Boolean;
var
   wNewTime : TTime;
   wStr : string;
   wMs, wh, wm, ws : word;
begin
   ...
   case fTimeDisplay of
     ...
     td12Hour:
       begin
          if (selstart = 2) or (selstart = 5) or (selstart = 8) then
             selstart := selstart + 1;
          case SelStart of
            0, 1, 3, 4:
            begin
