...
type
  ...
  TrmCustomNotebookControlEditor = class(TDefaultEditor)
...
{ TrmCustomNotebookControlEditor }

procedure TrmCustomNotebookControlEditor.ExecuteVerb(Index: Integer);
var
  NotebookControl: TrmCustomNotebookControl;
  Page: TrmNotebookPage;
begin
  if Component is TrmNotebookPage then
    NotebookControl := TrmNotebookPage(Component).NotebookControl
  else
    NotebookControl := TrmCustomNotebookControl(Component);
...
function TrmCustomNotebookControlEditor.GetVerb(Index: Integer): string;
begin
  case Index of
    0 : Result:= StrAddPage;
...
function TrmCustomNotebookControlEditor.GetVerbCount: Integer;
begin
  Result := 3;
end;
