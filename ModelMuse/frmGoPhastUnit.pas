...
type
  ...
  TfrmGoPhast = class(TUndoForm)
    ...
    procedure ShowGridValues1Click(Sender: TObject);
  ...
  private
    ...
    FCursorY: integer;
    // @name: TObjectList;
    // See @link(DeletedDataSets).
    FDeletedDataSets: TObjectList;
    // @name: @link(TFileFormat);
    // See @link(FileFormat).
    ...
    // If @link(frmProgress), @link(frmSelectedObjects), or @link(frmColors)
  ...
  public
    ...
    // The intent is to allow them to be restored later.  However, that
   ...
   // @name causes the the top view of the model to be redrawn.
    ...
    // @name is the @link(TPhastModel.PhastGrid) of @link(PhastModel).
              ...
              msModflow:
                begin
                  PhastModel.ExportModflowModel(
                    ChangeFileExt(FileName, '.nam'), False);
...
begin
  ...
  if acExportPhastInputFile.Enabled then
  begin
    acExportPhastInputFile.ShortCut := ShortCut(Word('E'), [ssCtrl]);
    tbRun.Action := acExportPhastInputFile;
  end
  else
  begin
    acExportPhastInputFile.ShortCut := 0;
  end;

  if acRunModflow.Enabled then
  begin
    acRunModflow.ShortCut := ShortCut(Word('E'), [ssCtrl]);
    tbRun.Action := acRunModflow;
  end
  else
  begin
    acRunModflow.ShortCut := 0;
  end;

  miLayers.Enabled := PhastModel.ModelSelection = msModflow;
  EnableHufMenuItems;
  miGeneral.Enabled := PhastModel.ModelSelection = msModflow;
  miTime.Enabled := PhastModel.ModelSelection = msModflow;
...
begin
  miMF_HydrogeologicUnits.Enabled := (PhastModel.ModelSelection = msModflow)
...
begin
  miManageFluxObservations.Enabled :=
    ...
    and (PhastModel.ModflowPackages.ChobPackage.IsSelected
...
begin
  ...
  if tbAddLinesToObjects.Down then
  begin
    // Make sure all buttons except the current one are up.
...
begin
  ...
  if tbAddPointsToObject.Down then
  begin
    // Make sure all buttons except the current one are up.
...
begin
  ...
  if tbAddPartsToObject.Down then
  begin
    // Make sure all buttons except the current one are up.
...
procedure TfrmGoPhast.ShowGridValues1Click(Sender: TObject);
begin
  inherited;
  if frmGridValue = nil then
  begin
  ...
  case Key of
    ...
    Key_Up:
      begin
        if BigJump then
        begin
          Grid.SelectedRow := Grid.SelectedRow + ReverseFactor*10;
        end
        else
        begin
          Grid.SelectedRow := Grid.SelectedRow + ReverseFactor*1;
        end;
    ...
    Key_PageUp:
      begin
        if BigJump then
        begin
          Grid.SelectedLayer := Grid.SelectedLayer + ReverseFactor*10;
        end
        else
        begin
          Grid.SelectedLayer := Grid.SelectedLayer + ReverseFactor*1;
        end;
...
begin
  ...
  MoveToTopCell((Grid.ColumnCount - 1) div 2,
    (Grid.RowCount - 1) div 2);
  MoveToFrontCell((Grid.ColumnCount - 1) div 2,
...
begin
  for Index := 0 to PhastModel.DataSetCount - 1 do
  begin
    ADataSet := PhastModel.DataSets[Index];
    ADataSet.Invalidate;
  end;
  for Index := 0 to PhastModel.BoundaryDataSetCount - 1 do
  begin
    ADataSet := PhastModel.BoundaryDataSets[Index];
    ADataSet.Invalidate;
  end;
...
begin
  SetButtonsUp(nil);
  SetZB_Cursors(crArrow);
  if (ModelSelection = msModflow) and
...
begin
  inherited;
  FRunModflowForm := TfrmRunModflow.createfordialog(sdModflowInput);
...
begin
  ...
  begin
    ExportFile(sdPhastInput.FileName, FRunPhast);
  end;
end;
