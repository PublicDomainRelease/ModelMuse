...
resourcestring
  ...
  StrImportFootprintRes = 'Import Footprint results';
  ...
  begin
    ...
    try
      ...
      try
        ...
        case comboColorContourGrid.ItemIndex of
          0:
            begin
              DataArray := nil;
            end;
...
begin
  if chklstDataToImport.Checked[Ord(fpdtWithdrawals)] then
  begin
    NewName := GenerateNewName(chklstDataToImport.Items[Ord(fpdtWithdrawals)]);
    NewDataType := rdtDouble;
    NewFormula := '0.';
    Classification := 'Model Results|Footprint';
...
begin
  if chklstDataToImport.Checked[Ord(fpdtCode)] then
  begin
    NewName := GenerateNewName(chklstDataToImport.Items[Ord(fpdtCode)]);
