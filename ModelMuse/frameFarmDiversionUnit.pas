...
begin
  inherited;
  if (ARow >= 1) and (ACol = Ord(docVertex)+2) then
  begin
    CanSelect := Grid.ItemIndex[Ord(docChoice)+2,ARow] = Ord(dpMiddle);
  end;
end;
...
//  TDiversionLocationColumns = (dlcX, dlcY);
            ...
            rtCell:
              begin
                DiversionCell := LinkedStream.DiversionCell;
                DiversionCell.Row :=
                  StrToIntDef(Grid.Cells[Ord(dccRow)+2,TimeIndex], 0);
                DiversionCell.Col :=
...
begin
  ...
  try
    ...
    begin
      ...
      Grid.Columns[ColIndex].ButtonCaption := 'F()';
