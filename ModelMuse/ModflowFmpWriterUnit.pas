...
begin
  frmErrorsAndWarnings.AddWarning(Model, NoDefinedErrorRoot,
...
procedure TModflowFmpWriter.WriteConstantU2DREL(const Comment: string;
  const Value: double);
begin
  case FWriteLocation of
    wlMain: inherited;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        STime := StepIndex/LenSim + StartingTime;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        WriteBooleanValueFromGlobalFormula(ACrop.Fallow, ACrop, 'Irrigated');
        WriteString(' # Data Set 17: Crop-ID IFALLOW');
        NewLine;
      end;
    finally
...
begin
  ...
  begin
    ...
    try
      WriteTransient2DArray(Comment, DataTypeIndex, DataType, DefaultValue,
        RefEtList, umAssign, False, Dummy);
    finally
      FWriteLocation := wlMain;
    end;
  end;
...
begin
  ...
  if IFWLCB = 1 then
  begin
    Model.AddModelInputFile(FileDir + 'FWELLS.OUT');
  end;
  {$REGION 'IFNRCB'}
  ...
  case IFNRCB of
    1:
      begin
        Model.AddModelInputFile(FileDir + 'FNRCH_ARRAY.OUT');
      end;
    2:
      begin
    ...
    3:
      begin
        Model.AddModelInputFile(FileDir + 'FNRCH_LIST_BIN.OUT');
      end;
  end;
  {$ENDREGION}
...
//      end;
  ...
  if ISDPFL = 1 then
  begin
    Model.AddModelInputFile(FileDir + 'FDS.OUT');
  end
  else if ISDPFL > 1 then
  begin
    WriteToNameFile(StrDATABINARY, ISDPFL, ChangeFileExt(FNameOfFile, '.FDS_BIN'), foOutput);
  end;
  {$ENDREGION}
  ...
  case IFBPFL of
    1:
      begin
        Model.AddModelInputFile(FileDir + 'FB_COMPACT.OUT');
      end;
    2:
      begin
        Model.AddModelInputFile(FileDir + 'FB_DETAILS.OUT');
      end;
    else
      begin
        if IFBPFL > 2 then
        begin
          ...
          begin
            WriteToNameFile(StrDATABINARY, IFBPFL,
              ChangeFileExt(FNameOfFile, '.FB_COMPACT_BIN_OUT'), foOutput);
          end
          else
          begin
            WriteToNameFile(StrDATABINARY, IFBPFL,
              ChangeFileExt(FNameOfFile, '.FB_DETAILS_BIN_OUT'), foOutput);
          end;
        end;
      end;
  end;
  ...
  begin
    ...
    begin
      Model.AddModelInputFile(FileDir + 'ROUT.OUT');
    end;
    {$ENDREGION}
    WriteInteger(IRTPFL);
  ...
  begin
    ...
    if IOPFL > 0 then
    begin
      Model.AddModelInputFile(FileDir + 'ACR_OPT.OUT');
    end;
    {$ENDREGION}
    WriteInteger(IOPFL);
  ...
  begin
   ...
   IPAPFL := Dummy;
    ...
    if IPAPFL = 1 then
    begin
      Model.AddModelInputFile(FileDir + 'PRIOR.OUT');
    end;
    {$ENDREGION}
    WriteInteger(IPAPFL);
...
begin
  ...
  begin
    ...
    try
      WriteTransient2DArray(Comment, DataTypeIndex, DataType, DefaultValue,
        PrecipList, FFarmProcess.AssignmentMethod, False, Dummy);
    finally
      FWriteLocation := wlMain;
    end;
  end;
