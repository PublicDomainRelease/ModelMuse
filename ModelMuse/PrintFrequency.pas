...
type
  ...
  private
    ...
    // See @link(Rates).
    procedure SetRates(const Index: TPrintFrequencyRows;
      const Value: double);
    // See @link(SaveFinalHeads).
    ...
    // See @link(Units).
    procedure SetUnits(const Index: TPrintFrequencyRows;
  ...
  public
    ...
    property Rates[const Index: TPrintFrequencyRows]: double read GetRates
      write SetRates;
    // @name retrieves the units for a rate based on Index.
    // See @link(TPrintFrequencyRows).
    property Units[const Index: TPrintFrequencyRows]: TFrequencyUnits
...
function TPrintFrequencyItem.GetRates(
  const Index: TPrintFrequencyRows): double;
begin
  result := 0;
  case Index of
    pfrFlowRate:
...
function TPrintFrequencyItem.GetUnits(
  const Index: TPrintFrequencyRows): TFrequencyUnits;
begin
  result := fuDefault;
  case Index of
    pfrFlowRate:
...
procedure TPrintFrequencyItem.SetRates(const Index: TPrintFrequencyRows;
  const Value: double);
begin
  case Index of
    pfrFlowRate:
...
procedure TPrintFrequencyItem.SetUnits(const Index: TPrintFrequencyRows;
  const Value: TFrequencyUnits);
begin
  case Index of
    pfrFlowRate:
