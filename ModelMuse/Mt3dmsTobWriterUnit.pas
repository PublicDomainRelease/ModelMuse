...
resourcestring
  ...
  ConcObsNameWarning = 'The following Transport observation names may be '
    + 'valid for MT3DMS but they are not valid for UCODE.';
...
begin
  // Write data set 8
  MaxObsTimeLength := Length(IntToStr(ObsGroup.ObservationTimes.Count));
...
begin
  ...
  if inConcObs > 0 then
  begin
    WriteToMt3dMsNameFile(StrData, Mt3dTob_inConcObs, ChangeFileExt(FNameOfFile, '.ocn'));
  end;
  if inFluxObs > 0 then
  begin
    WriteToMt3dMsNameFile(StrData, Mt3dTob_inFluxObs, ChangeFileExt(FNameOfFile, '.mfx'));
  end;
  if inSaveObs > 0 then
  begin
    WriteToMt3dMsNameFile(StrDataBinary, Mt3dTob_inSaveObs, ChangeFileExt(FNameOfFile, '.pst'));
  end;
