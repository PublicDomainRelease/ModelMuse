  ...
  begin
    ...
    begin
      case TSwrConnectionColumns(ACol) of
        sccObjectName:
          begin
            CanSelect := (TSwrConnectionMethod(ItemIndex) = scmObject)
          end;
        sccReach:
          begin
...
begin
  inherited;
  frameSwr.dgModflowBoundarySelectCell(Sender, ACol, ARow, CanSelect);
  if (ARow >= frameSwr.dgModflowBoundary.FixedRows) and (ACol = Ord(srcStage)) then
  begin
    CanSelect := (ARow = 1)
      or (frameSwr.dgModflowBoundary.ItemIndex[Ord(srcReachType), ARow] = 2)
...
begin
  inherited;
  frameSwr.dgModflowBoundarySetEditText(Sender, ACol, ARow, Value);
  if (ARow > 1) and (ACol = Ord(srcReachType)) then
  begin
    frameSwr.dgModflowBoundary.Invalidate;
  end;
end;
...
begin
  ...
  try
    ...
    frameSwr.dgModflowBoundary.Columns[Ord(srcGeomName)].PickList := PickList;
  finally
    PickList.Free;
  end;
  ...
  ClearGrid(frameSwr.dgModflowBoundary);
  ...
  try
    ...
    begin
      ...
      Grid := frameSwr.dgModflowBoundary;
...
begin
  ...
  begin
    frameSwr.dgModflowBoundary.ColCount := 5;
  end
  else
  begin
    frameSwr.dgModflowBoundary.ColCount := 6;
    Column := frameSwr.dgModflowBoundary.Columns[
      frameSwr.dgModflowBoundary.ColCount-1];
    ...
    Column.ButtonCaption := 'F()';
    Column.ButtonWidth := 35;
  end;

  frameSwr.dgModflowBoundary.Cells[Ord(srcStartTime), 0] := StrStartingTime;
  frameSwr.dgModflowBoundary.Cells[Ord(srcEndTime), 0] := StrEndingTime;
  frameSwr.dgModflowBoundary.Cells[Ord(srcGeomName), 0] := StrGeometryIGEONUMR;
  frameSwr.dgModflowBoundary.Cells[Ord(srcVerticalOffset), 0] := StrVerticalOffsetGZS;
  frameSwr.dgModflowBoundary.Cells[Ord(srcReachType), 0] := StrReachTypeISWRBND;
  if Ord(srcStage) < frameSwr.dgModflowBoundary.ColCount then
  begin
    frameSwr.dgModflowBoundary.Cells[Ord(srcStage), 0] := StrStageSTAGE;
  end;
