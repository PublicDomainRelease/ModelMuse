...
type
  ...
  private
    ...
    procedure WriteTransientSwrArray(CellList: TList; AssignmentMethod: TUpdateMethod; Comment: string);
      ...
      begin
        ...
        try
          ...
          (ScreenObject.Delegate as TModflowDelegate).InitializeExpression(
            Compiler, DataSetFunction, ReachLengthExpression, nil,
             DataToCompile);
          ...
          DataToCompile.AlternateDataType := rdtInteger;
          (ScreenObject.Delegate as TModflowDelegate).InitializeExpression(
            Compiler, DataSetFunction, ObservationExpression, nil,
             DataToCompile);
...
begin
  ...
  WriteTransient2DArray(Comment, DataTypeIndex, DataType, DefaultValue,
    CellList, AssignmentMethod, True, Dummy);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if ISTRTYPE in [3, 6, 7, 8, 9, 10, 12] then
      begin
      ...
      begin
        ISTRORCH := AStructure.ConnectedReach;
...
begin
  ...
  begin
    TransientCell := ValueCellList[CellIndex] as TSwrTransientCell;
    if (TimeIndex = 0) or (TransientCell.ReachType = srtSpecifiedHead) then
    begin
      Reach := FReachList[CellIndex];
      WriteInteger(Reach.FReachData.Reach);
      ISWRBND := MaxInt;
...
begin
  ...
  WriteToNameFile(StrSWR, Model.UnitNumbers.UnitNumber(StrSWR), FNameOfFile, foInput);
    ...
    try
      ...
      begin
        ...
        WriteTransient2DArray(' # Data Set 2: DROMAP2D' , DataTypeIndex,
          DataType, DefaultValue, CellList, umAssign, True, Dummy);
  ...
  case FSwrPackage.PrintInflowsAndOutflows of
    ...
    spoASCII:
      begin
        ...
        WriteToNameFile(StrData, OutputUnitNumber, OutputFileName, foOutput);
    ...
    spoBinary:
      begin
        ...
        OutputUnitNumber := Model.UnitNumbers.UnitNumber(StrSwrReachGroupStage);
        WriteToNameFile(StrDataBinary, OutputUnitNumber, OutputFileName, foOutput);
        ISWRPRGF := -OutputUnitNumber;
      end;
    else
      Assert(False);
...
//        if Model.SwrObservations.Count > 0 then
    ...
    if FDirectRunoffValues.Count > 0 then
    begin
      FDirectRunoffFileName := ChangeFileExt(FNameOfFile, StrSwrDirectRunoff);
      OutputUnitNumber := Model.UnitNumbers.UnitNumber(StrSwrDirectRunoff);
      WriteToNameFile(StrData, OutputUnitNumber, FDirectRunoffFileName, foInput);
