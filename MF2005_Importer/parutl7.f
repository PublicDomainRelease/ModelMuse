...
C9C-----Get zone numbers.
   ...
   32       CONTINUE
            IF(ILFLG.NE.0) THEN
	        WRITE(IOUT, *) 
	1  		  'MULTIPLIER ARRAY AND ZONE ARRAY FOR A LAYER:'
...
!              WRITE(IOUT,36) IPCLST(1,I),LINE(IM1:IM2),LINE(IZ1:IZ2)
 !  36         FORMAT(16X,'LAYER: ',I3,'    MULTIPLIER ARRAY: ',A,
  !   2        '    ZONE ARRAY: ',A)
            ELSE
	        WRITE(IOUT, *) 'MULTIPLIER ARRAY AND ZONE ARRAY:'
	        WRITE(IOUT, *) LINE(IM1:IM2)
	        WRITE(IOUT, *) LINE(IZ1:IZ2)
...
C1------Read the parameter name and definition.
	...
	WRITE(IOUT, *) 'PARTYP:'
	WRITE(IOUT, *) PTYP
	WRITE(IOUT, *) 'Parval,NLST:'
	WRITE(IOUT, *) PV, NLST
C
C2------Check for multiple instances.
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,N,R,IOUT,IN)
      IF (LINE(ISTART:ISTOP).EQ.'INSTANCES') THEN
	  WRITE(IOUT, *) 'INSTANCES:'
        CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,NUMINST,R,IOUT,IN)
	  WRITE(IOUT, *) 'NUMINST:'
	  WRITE(IOUT, *) NUMINST
        IF (NUMINST.LT.1) THEN
