...
C-------SET FLAG FOR NWT
      INWTUNIT = Inwt
C
C-------IDENTIFY OPTION ANDINITIALIZE # OF GROUP ROUTING ELEMENTS
      WRITE(IOUT,*) 'SWR:'
...
C         PROBLEM DIMENSION AND OUTPUT CONTROL - INPUT ITEM 1
      ...
      WRITE(IOUT,*)
     1 'NREACHES,ISWRONLY,ISWRCBC,ISWRPRGF,ISWRPSTG,ISWRPQAQ,ISWRPQM,
     2    ISWRPSTR,ISWRPFRN:'
      WRITE(IOUT,*)
     1 NREACHES,ISWRONLY,ISWRCB,ISWRPRGF,ISWRPSTG,ISWRPQAQ,ISWRPQM,
     2    ISWRPSTR,ISWRPFRN
...
!          WRITE (IOUT,2001)
          POPTIONSL: DO i = 1, (nopt - 1)
            ...
            IF ( j.NE.0 ) THEN
              ...
              END IF
              WRITE(IOUT, *) 'CSWROPT:'
              WRITE(IOUT, *) swroptions(i)%coption
              IF ( swroptions(i)%iunit.NE.0 ) THEN
                WRITE(IOUT, *) 'IOPTUNIT:'
                WRITE(IOUT, *) swroptions(i)%iunit
              endif
!              WRITE ( IOUT,2002)
...
!      CALL SSWR_GET_MODFLOW_TIME(Nmaxper,Ngetstp,Rt,Rtot)
      ...
      IF ( ITSPMAX.EQ.1 ) THEN
        ...
        END IF
      END IF
      WRITE(IOUT, *) 'DLENCONV, TIMECONV, RTINI, RTMIN, RTMAX, RTPRN, 
     1   RTMULT, NTMULT, DMINGRAD, DMNDEPTH:'      
...
!        IADTIME = 1
      IF ( IADTIME.EQ.1 ) THEN
        ...
        WRITE(IOUT,*) 'DMAXRAI,DMAXSTG,DMAXINF:'
        WRITE(IOUT,*) DMAXRAI,DMAXSTG,DMAXINF
      END IF
C
...
C         SET ROUTING TYPE
     ...
     2                 'SPECIFIED - REVISE SWR1 DATASET 4A')
        END SELECT
        WRITE(IOUT, *) 'IRCH4A IROUTETYPE IRGNUM KRCH IRCH JRCH RLEN:'
        WRITE(IOUT, *) REACH(irch)%IREACH, REACH(irch)%IROUTETYPE,
     1     REACH(irch)%IRGNUM, REACH(irch)%KRCH, REACH(irch)%IRCH,
     2     REACH(irch)%JRCH, REACH(irch)%DLEN
      END DO
...
C         IMPLICIT QM MAGNITUDE
        ...
        END IF
        WRITE(IOUT,*) 'IRCH4B NCONN:'
        WRITE(IOUT,*) irch,nconn
        DO n = 1, nconn
...
C---------DATASET 4D - TABULAR DATA SPECIFICATIONS
          ...
          DO j = 1, NTABS
            ...
            WRITE(IOUT,*) 'ITABUNIT:'
            WRITE(IOUT,*) TABDATA(itab)%ITABUNIT
            CALL URWORD(line, lloc, istart, istop, 0, ival, r, IOUT, In)
            SELECT CASE ( line(istart:istop) )
              CASE ('NONE')
...
C             TABULAR RAINFALL, EVAPORATION, LATERAL INFLOW, AND STAGE DATA
     ...
     2              ntabrch, (TABDATA(itab)%ITABRCH(i),i = 1,ntabrch)
                  WRITE(IOUT,*) 'ntabrch ITABRCH:'
                  WRITE(IOUT,*) ntabrch
                  DO I = 1, ntabrch
                    WRITE(IOUT,*) TABDATA(itab)%ITABRCH(i)
                  ENDDO
              END SELECT
            END IF
C
...
C           READ REACH NUMBER
     ...
     2                        i, 'REACH', irch, NREACHES
          ...
          WRITE(IOUT,*) 'IBNDRCH ISWRBND:'
          WRITE(IOUT,*) irch, REACH(irch)%ISWRBND
...
C         READ REACH NUMBER
     ...
     2                        i, 'REACH', irch, NREACHES
          ...
          WRITE(IOUT,*) 'IRAIRCH RAIN;'
          WRITE(IOUT,*) irch, REACH(irch)%RAIN
        END DO
C         CLEAN UP TEMPORARY LIST STORAGE
        DEALLOCATE (rlist)
C         2D RAINFALL DATA
...
C         READ REACH NUMBER
     ...
     2                        i, 'REACH', irch, NREACHES
          ...
          WRITE(IOUT,*) 'IEVPRCH EVAP;'
          WRITE(IOUT,*) irch, REACH(irch)%EVAP
        END DO
C         CLEAN UP TEMPORARY LIST STORAGE
        DEALLOCATE (rlist)
...
C         READ REACH NUMBER
     ...
     2                        i, 'REACH', irch, NREACHES
          ...
          WRITE(IOUT,*) 'ILINRCH QLATFLOW;'
          WRITE(IOUT,*) irch, REACH(irch)%QLATFLOW
        END DO
C         CLEAN UP TEMPORARY LIST STORAGE
        DEALLOCATE (rlist)
...
C         READ REACH NUMBER
     ...
     3                   i, 'REACH', irch, NREACHES
          ...
          IF ( ival.GT.IZERO ) THEN
            ...
            WRITE(IOUT,*) 'IGMODRCH IGEONUMR GZSHIFT:'
            WRITE(IOUT,*) irch, REACH(irch)%IGEONUM, 
     1           REACH(irch)%GZSHIFT
          END IF
        END DO
C         CLEAN UP TEMPORARY LIST STORAGE
        DEALLOCATE (rlist)
...
C           READ GENERIC GEOMETRY DATA        
          ...
          gmanning = SSWR_R2D(r)
          WRITE(IOUT,*) 'IGEONUM IGEOTYPE IGCNDOP GMANNING:'
          WRITE(IOUT,*) IGEONUM, IGEOTYPE, IGCNDOP, GMANNING
...
C         DATA SET 12
     ...
     3                  i, 'REACH', irch, NREACHES
          ...
          END IF
          ival = INT(rlist(2,i))
          WRITE(IOUT,*) 'ISMODRCH NSTRUCT:'
          WRITE(IOUT,*) irch, ival
          IF ( ival.GT.IZERO ) THEN
...
C           GET STRUCTURE TYPE
          ...
          WRITE(IOUT,*) 'ISTRRCH ISTRNUM ISTRCONN ISTRTYPE :'
          WRITE(IOUT,*) ISTRRCH, ISTRNUM, ISTRCONN, ISTRTYPE 
...
C                 CRITICAL-DEPTH BOUNDARY CONDITION
              ...
              IF ( istrtype.LT.0 ) THEN
                ...
                WRITE(IOUT,*) 'STRINV:'
                WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%STRINV 
...
C             SPECIFIED DISCHARGE
            CASE (3)
              ...
              WRITE(IOUT,*) 'STRVAL:'
              WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%STRVAL
...
C             SPECIFIED H-Q RELATION
            CASE (4)
              ...
              WRITE(IOUT,*) 'NSTRPTS:'
              WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%NSTRPTS
...
C               CULVERT
     ...
     4          SSWR_R2D(r)
              ...
              WRITE(IOUT,*) 'STRLEN, STRMAN, ISTRDIR:'
              WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%STRLEN,
     1          REACH(istrrch)%STRUCT(istrnum)%STRMAN,
     2          REACH(istrrch)%STRUCT(istrnum)%ISTRDIR
...
C               UPDATE GATE BOTTOM
     ...
     2          REACH(istrrch)%STRUCT(istrnum)%STRINV,
     3          REACH(istrrch)%STRUCT(istrnum)%STRWID,
     4          REACH(istrrch)%STRUCT(istrnum)%STRVAL,
     5          REACH(istrrch)%STRUCT(istrnum)%ISTRDIR
...
C           ADDITIONAL BOUNDARY STRUCTURE DATA FOR COUPLING TO SFR
          IF ( istrconn.EQ.0 .AND. ISFRUNIT.GT.0 ) THEN
            ...
            WRITE(IOUT,*) 'ISFRSEG:'
            WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%ISFRSEG
            IF ( ival.GT.0 ) THEN
              CALL URWORD(line,lloc,istart,istop,2,ival,r,IOUT,iut)
              REACH(istrrch)%STRUCT(istrnum)%ISFRRCH = ival
              WRITE(IOUT,*) 'ISFRRCH:'
              WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%ISFRRCH
...
C           DATASET 13B - STRUCTURE OPERATION DATA
     ...
     2                     'VARIABLE.')
            ...
            ELSE
              ...
              WRITE(IOUT,*) 'ISTRORCH:'
              WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%ISTRORCH
     ...
     2                     '0 AND LESS THAN NREACHES + 1')
              ...
              IF ( REACH(istrrch)%STRUCT(istrnum)%ISTROVAL.EQ.2 ) THEN
                ...
                WRITE(IOUT,*) 'ISTROQCON:'
                WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%ISTRORCHC
              END IF
              CALL URWORD(line,lloc,istart,istop,0,ival,r,IOUT,iut)
              SELECT CASE ( line(istart:istop) )
                CASE ('LT')
     ...
     2                       'OPERAND (LOPR).')
            ...
            IF ( indx.GT.0 ) THEN
              ...
              WRITE(IOUT,*) 'TABDATA, ISTRTAB:'
              WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%ISTRTAB
              r = DZERO
            END IF
            IF ( ird.EQ.0 ) THEN
     ...
     2                       'CSTROTYP=STAGE')
                ...
                WRITE(IOUT,*) 'REACH, ISTROSTG:'
                WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%ISTROSTG
                r = DZERO
              END IF
            END IF
            IF ( ird.EQ.0 ) THEN
            ...
            WRITE(IOUT,*) 'STRCRIT:'
            WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%STRCRIT
            IF ( istrtype.GT.1 ) THEN
              CALL URWORD(line,lloc,istart,istop,3,ival,r,IOUT,iut)
              REACH(istrrch)%STRUCT(istrnum)%STRCRITC = SSWR_R2D(r)
              CALL URWORD(line,lloc,istart,istop,3,ival,r,IOUT,iut)
              REACH(istrrch)%STRUCT(istrnum)%STRRT = SSWR_R2D(r)
              WRITE(IOUT,*) 'STRCRITC, STRRT:'
              WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%STRCRITC,
     1          REACH(istrrch)%STRUCT(istrnum)%STRRT
            END IF
            CALL URWORD(line,lloc,istart,istop,3,ival,r,IOUT,iut)
            REACH(istrrch)%STRUCT(istrnum)%STRMAX = SSWR_R2D(r)
              WRITE(IOUT,*) 'STRMAX:'
              WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%STRMAX
            CALL URWORD(line,lloc,istart,istop,0,ival,r,IOUT,iut)
            IF ( istrtype.GT.1 ) THEN
              indx = INDEX(line(istart:istop),'TABDATA')
              IF ( indx.GT.0 ) THEN
...
C             READ REACH NUMBER
            ...
            IF ( irchstg.GT.0 .AND. irchstg.LE.NREACHES ) THEN
              ...
              RCHGRP(irg)%IRGUPDATE = 1
              WRITE(IOUT,*) 'IRCHSTG STAGE:'
              WRITE(IOUT,*) irchstg, dstage(i)
            END IF
          END DO
          DEALLOCATE (rlist)
        ELSE
...
C           PROCESS 2D DATA
          DO irch = 1, NREACHES
            ...
            RCHGRP(irg)%IRGUPDATE = 1
              WRITE(IOUT,*) 'IRCHSTG STAGE:'
              WRITE(IOUT,*) istage(irch), dstage(irch)
          END DO
