...
C             OBSERVATION TYPE
            ...
            WRITE(IOUT,*) 'COBSTYPE:'
            WRITE(IOUT,*) OBSDATA(j)%COBSTYPE
            CALL URWORD(line, lloc, istart, istop, 2, ival, r, IOUT, In)
            OBSDATA(j)%IOBSLOC = ival
            WRITE(IOUT,*) 'IOBSLOC:'
            WRITE(IOUT,*) OBSDATA(j)%IOBSLOC
            SELECT CASE(OBSDATA(j)%IOBSTYPE)
              CASE (10:)
                CALL URWORD(line,lloc,istart,istop,2,ival,r,IOUT,In)
                OBSDATA(j)%IOBSLOC2 = ival
                WRITE(IOUT,*) 'IOBSLOC2 or IOBSLAY:'
                WRITE(IOUT,*) OBSDATA(j)%IOBSLOC2
            END SELECT
          END DO
        END IF
      ELSE
...
C               TEST USE OF STRLEN AND STRLEN2 FOR UNCONTROLLED FLOW STRUCTURES - JDH 11/28/2012
     ...
     2              REACH(istrconn)%DLEN
                ELSE
                  ...
                  WRITE(IOUT,*) 'STRLEN2:'
                  WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%STRLEN2
                END IF
              END IF
...
C             SPECIFIED H-Q RELATION
     ...
     2            SSWR_R2D( rlist(2,n) )
                WRITE(IOUT,*) 'STRELEV STRQ:'
                WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%STRELEV(n),
     1            REACH(istrrch)%STRUCT(istrnum)%STRQ(n)
              END DO
C               CLEAN UP TEMPORARY LIST STORAGE
              DEALLOCATE (rlist)
C               CULVERT
...
C           DATASET 13B - STRUCTURE OPERATION DATA
     ...
     2                       'OPERAND (LOPR).')
            ...
            IF ( indx.GT.0 ) THEN
              ...
              READ (line(istart+7:istop),*) ival
              REACH(istrrch)%STRUCT(istrnum)%ISTRTAB = ival
              WRITE(IOUT,*) 'TABDATA CSTRCRIT ISTRTAB:'
              WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%ISTRTAB
              r = DZERO
            END IF
            IF ( ird.EQ.0 ) THEN
              indx = INDEX(line(istart:istop),'REACH')
              ! RBW: This option is not documented. Aug 12, 2014.
              IF ( indx.GT.0 ) THEN
     ...
     2                       'STRCRIT AND STRVAL.')
                ...
                READ (line(istart+7:istop),*) ival
                REACH(istrrch)%STRUCT(istrnum)%ISTRTAB     = ival
                WRITE(IOUT,*) 'TABDATA CSTRVAL ISTRTAB:'
                WRITE(IOUT,*) REACH(istrrch)%STRUCT(istrnum)%ISTRTAB
              END IF
            END IF
            ival = REACH(istrrch)%STRUCT(istrnum)%ISTRTAB
